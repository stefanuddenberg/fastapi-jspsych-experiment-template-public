...
2. Set up your Python environment with uv: `uv venv`
3. Install the project dependencies: `uv sync`
4. Initialize/reset the local database: `uv run cli.py reset_db`
   - You can also do this by activating your Python environment (e.g., `source env/bin/activate`) and running `python cli.py reset_db`
5. Install frontend packages: `uv run cli.py install_packages` OR from within `frontend/`: `npm i`
6. In the `frontend/` folder, add a blank `client.env` file, just to get it to work for now
...
## Run locally
- In one terminal, start the frontend server: `uv run python cli.py debug` OR from within `frontend/`: `npm run dev`
- In another, separate terminal, start the backend server: `uv run python cli.py run`
...
## Export your data
`railway run python cli.py export` # assuming you have your Python environment activated
